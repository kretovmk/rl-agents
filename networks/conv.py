...
class NetworkCategorialConvKeras(NetworkBase):
    ...
    def _build_network(self):
        ...
        out = inputs
        out = Convolution2D(8, 8, 8, subsample=(2, 2), activation='relu', border_mode='same', dim_ordering='th')(out)
...
class NetworkRegConvKeras(NetworkBase):
    ...
    def _build_network(self):
        inp = tf.placeholder(shape=(None,) + self.inp_shape, dtype=tf.float32, name='qqqqq2')
        targets = tf.placeholder(shape=(None, self.n_outputs), dtype=tf.float32)

        # keras model
        inputs = Input(shape=self.inp_shape, name='qqqqqqqq')
        out = inputs
        out = Convolution2D(8, 8, 8, subsample=(2, 2), activation='relu', border_mode='same', dim_ordering='th')(out)
        out = Convolution2D(16, 4, 4, subsample=(2, 2), activation='relu', border_mode='same', dim_ordering='th')(out)
        out = Flatten()(out)
